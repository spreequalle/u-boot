...
int   rt2880_eth_recv(struct eth_device* dev);
...
void rt305x_esw_init(void)
{
	...
	RT2882_REG(0xb0110098) = 0x00007f3f;
	RT2882_REG(0xb01100CC) = 0x00d6500c;
	RT2882_REG(0xb011009C) = 0x0008a301;
	...
	for(i=0;i<5;i++){
		mii_mgr_write(i, 26, 0x1600);
		mii_mgr_write(i, 29, 0x7058);
		mii_mgr_write(i, 30, 0x0018);
	}
	/* PHY IOT */
	mii_mgr_write(0, 31, 0x0);
	mii_mgr_write(0, 1, 0x4a40);
	mii_mgr_write(0, 2, 0x6254);
	mii_mgr_write(0, 3, 0xa17f);
	mii_mgr_write(0, 14, 0x65);
	mii_mgr_write(0, 16, 0x0684);
	mii_mgr_write(0, 17, 0x0fe0);
	mii_mgr_write(0, 18, 0x40ba);
	mii_mgr_write(0, 22, 0x052f);
	mii_mgr_write(0, 27, 0x2fc3);
	mii_mgr_write(0, 28, 0xc410);
	mii_mgr_write(0, 29, 0x598b);
	mii_mgr_write(0, 31, 0x8000);
	for(i=0;i<5;i++){
		//LSB=1 enable PHY
	...
	RT2882_REG(0xb01100C8) &= ~(1<<29);
	RT2882_REG(0xb01100C8) |= 0x3fff;
	RT2882_REG(0xb01100C8) &= ~(0xf<<20);
#elif defined (P5_MII_TO_MAC_MODE)
	RT2882_REG(0xb01100C8) &= ~(1<<29);
	RT2882_REG(0xb01100C8) |= 0x3ffd;
	...
	RT2882_REG(0xb0000674) = 0xFFF;
	RT2882_REG(0xb0000670) = 0x0;
