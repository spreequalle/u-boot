...
#define RALINK_GPIO_14				0x00004000	// WPS LED

#define WLAN_LED						(RALINK_11N_MAC_BASE + 0x102c)
#define LED_PORT0						(RALINK_ETH_SW_BASE + 0xA4)
	...
	check_image_validation();
	...
	
		{
			...
			*(volatile u32 *)(RALINK_REG_GPIOMODE) = cpu_to_le32(gpio_mode);
			...
			gpio_mode = RALINK_GPIO_DIR_ALLIN | RALINK_GPIO_8 | RALINK_GPIO_9 | RALINK_GPIO_14;			
			*(volatile u32 *)(RALINK_REG_PIODIR) =	cpu_to_le32(gpio_mode);
			
			io_data = RALINK_GPIO_8 | ~RALINK_GPIO_9 | RALINK_GPIO_14;					
			*(volatile u32 *)(RALINK_REG_PIODATA) = cpu_to_le32(~io_data);	// turn on WPS LED									
			*(volatile u32 *)(WLAN_LED) = cpu_to_le32(0x40FFFFFF);	//  turn on wireless led		
			*(volatile u32 *)(LED_PORT0) = cpu_to_le32(0x0C);	// turn on Port0
			
			udelay(1000000);
			
			io_data = RALINK_GPIO_8 | ~RALINK_GPIO_9 | RALINK_GPIO_14;
			*(volatile u32 *)(RALINK_REG_PIODATA) = cpu_to_le32(io_data);	// turn off WPS LED			
			*(volatile u32 *)(WLAN_LED) = cpu_to_le32(0x00FFFFFF);	//  turn off wireless led
			*(volatile u32 *)(LED_PORT0) = cpu_to_le32(0x05);	// turn off Port0
									
			io_data = le32_to_cpu(*(volatile u32 *)(RALINK_REG_PIODATA));
